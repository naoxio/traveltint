...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
            ...
            // Only check for country click if we haven't dragged much
            Vector2 endPos = GetMousePosition();
            float dragDistance = sqrt(pow(endPos.x - dragStart.x, 2) + pow(endPos.y - dragStart.y, 2));
            
            if (dragDistance < 5.0f) {  // Threshold for considering it a click
                for (int i = 0; i < map->countryCount; i++) {
                    Country* country = &map->countries[i];
                    Vector2* screenPoints = (Vector2*)malloc(country->pointCount * sizeof(Vector2));
                    
                    // Transform country points to screen coordinates
                    for (int j = 0; j < country->pointCount; j++) {
                        screenPoints[j] = (Vector2){
                            longitudeToScreenX(country->polygonPoints[j].x, map->zoom, map->offset.x),
                            latitudeToScreenY(country->polygonPoints[j].y, map->zoom, map->offset.y)
                        };
                    }

                    if (CheckCollisionPointPoly(endPos, screenPoints, country->pointCount)) {
        ...
        if (clickedCountry[0] != '\0') {
            DrawText(TextFormat("Clicked Country: %s", clickedCountry), 10, 90, 20, WHITE);
        }
